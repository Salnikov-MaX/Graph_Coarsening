...
import graph_coarsening.maxWeightMatching as maxWeightMatching
...
def coarsen(
    ...
    for level in range(1, max_levels + 1):
        ...
        if algorithm == "optimal":
            ...
            coarsening_list, err_list, r_list = matching_greedy(G, weights=weights, r=r_cur, k=K)
            print('Done!')
    ...
    return C, Gc, Call, Gall, coarsening_list, iC, err_list, r_list
...
def matching_greedy(G, weights, r=0.4, k=5, Uk=None, lk=None):
    """
    ...
    * Depending on G, the algorithm might fail to return ratios>0.3
    ...
    while len(candidate_edges) > 0:
        ...
        err_list.append(error_eigenvalue)
        r_list.append(metrics['r'])
        print(error_eigenvalue)

        # if error_eigenvalue > 0.18:
        #     bad_iter_num+=1
        #     continue
    ...
    return np.array(matching), err_list, r_list
